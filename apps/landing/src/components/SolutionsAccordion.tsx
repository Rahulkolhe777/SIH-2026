...
export default function SolutionsAccordion() {
  ...
  return (
    <section
      id="solutions"
      ref={sectionRef}
      className="w-full bg-[#FCFCFA] px-6 sm:px-8 md:px-12 lg:px-16 pt-4 pb-20 md:pb-28"
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start mb-12 md:mb-16">
          ...
          <div className="lg:col-span-7">
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-[54px] leading-[1.08] tracking-tight text-[#0B2D1B]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-stretch">
          ...
          <div className="lg:col-span-6 flex flex-col gap-3.5 justify-start">
              ...
              return (
                <div
                  ...
                  className={`w-full rounded-[16px] transition-all duration-300 cursor-pointer overflow-hidden border ${
                    isActive
                      ? "bg-[#F4F4F2] border-[#0B2D1B]/15 shadow-sm p-4 md:p-5"
                      : "bg-[#F4F4F2] hover:bg-[#ecece8] border-transparent p-4 md:p-4.5"
                  }`}
                >
                  {/* Header Row */}
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3.5">
                      {/* Icon Box */}
                      <div
                        className={`w-9 h-9 md:w-10 md:h-10 rounded-lg flex items-center justify-center transition-colors duration-300 shrink-0 ${
                          isActive
                      ...
                      {/* Title */}
                      <h3 className="text-base md:text-[17px] font-medium text-[#0B2D1B] tracking-tight">
                        {item.title}
                      </h3>
                    </div>

                    {/* Plus / Minus Indicator */}
                    <div className="text-[#0B2D1B] shrink-0 p-1">
                      {isActive ? (
                        <Minus size={18} strokeWidth={2.5} />
                      ) : (
                        <Plus size={18} strokeWidth={2.5} className="text-[#7A8B7F]" />
                      )}
                    </div>
                  </div>
                  ...
                  >
                    <div className="overflow-hidden">
                      <div className="bg-white rounded-xl p-4 md:p-4.5 text-sm md:text-[14.5px] text-[#5A6C5F] leading-relaxed border border-black/5 shadow-xs">
                        {item.description}
                      </div>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-6 relative min-h-[340px] sm:min-h-[420px] lg:min-h-full h-[420px] lg:h-auto rounded-2xl md:rounded-[24px] overflow-hidden shadow-md bg-[#E8EAE6]">
            {accordionData.map((item, index) => {
              const isActive = activeIndex === index;
              return (
                <div
